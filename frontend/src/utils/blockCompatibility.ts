...
export function isBlockCompatible(
  ...
  if (!rule) {
    ...
    console.warn(
      `⚠️ 未找到積木類別 ${category} 的相容性規則，使用智能預設規則`
    );
    console.log(
      "📋 可用的積木類別規則:",
  ...
  if (rule.restrictions?.forbiddenWith) {
    ...
    if (hasForbiddenBlock) {
      return {
        ...
        suggestions: [
          `請移除 ${rule.restrictions.forbiddenWith.join(" 或 ")} 積木`,
        ],
      };
    }
  }
...
export function getBlockUsageSuggestions(category: BlockCategory): string[] {
  ...
  if (rule?.restrictions?.requiresParent) {
    suggestions.push(
      `需要放置在 ${rule.restrictions.requiresParent.join(" 或 ")} 積木內`
    );
  }
...
export function validateWorkspace(
  ...
  blocks.forEach((block, index) => {
    ...
    if (!validation.isValid) {
      errors.push(
        `積木 ${index + 1} (${block.category}): ${validation.reason}`
      );
    }
  });
